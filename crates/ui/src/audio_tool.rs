//! Audio converter: drop audio (or video, to extract its audio) files, pick a
//! format + quality, run ffmpeg on the background executor with live progress.
...
use konvrt_core::audio::{
    AudioFormat, AudioOutcome, AudioQuality, convert_file, is_supported_input,
};
use konvrt_core::video::find_ffmpeg;
...
impl AudioTool {
    ...
    fn set_format(&mut self, format: AudioFormat, cx: &mut Context<Self>) {
        self.format = format;
    ...
    fn render_drop_zone(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        crate::dropzone::drop_zone(
            ...
            "or click to browse · mp3 · wav · flac · ogg · m4a · opus · or drop a video to extract its audio",
        )
